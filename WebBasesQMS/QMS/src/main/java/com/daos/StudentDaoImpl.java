...
public class StudentDaoImpl implements StudentDao {
	...
	public List<String[]> viewScore(int id) throws SQLException {
		...
		String sql = "select at.quiz_id,at.final_score, q.title from quizzes q inner join quiz_attempts at on q.quiz_id=at.quiz_id where student_id=?";
		try (PreparedStatement stmt = con.prepareStatement(sql)) {
			stmt.setInt(1, id);
	...
	public boolean checkAttempt(int student_id, int quiz_id) throws SQLException {
		String sql = "select at.quiz_id,at.final_score, q.title from quizzes q inner join quiz_attempts at on q.quiz_id=at.quiz_id where student_id=? and at.quiz_id=?";
		try (PreparedStatement stmt = con.prepareStatement(sql)) {
