...
public class QuizDao implements AutoCloseable {
	...
	public List<Question> takeQuiz(int id) throws SQLException {
		...
		String sql = "select question_text,option_a,option_b,option_c,option_d,correct_option from questions qns inner join quizzes q on q.quiz_id=qns.quiz_id where qns.quiz_id=? ";
		try(PreparedStatement stmt  =con.prepareStatement(sql)){
			stmt.setInt(1, id);
			ResultSet rs = stmt.executeQuery();
			
			while(rs.next()) {
				Question qz = new Question(id, rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6).charAt(0));
				list.add(qz);
			}
		}
		return list;
	...
	public void attempQuiz(int quiz_id,int user_id,int score,int question_count) throws SQLException {
		...
		String sql = "insert into quize_attempts(quiz_id,student_id,final_score,total_questions) values(?,?,?,?)";
		try(PreparedStatement stmt = con.prepareStatement(sql)){
